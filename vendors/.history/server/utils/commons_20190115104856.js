...
exports.runCaseScript = async function runCaseScript(params, colId, interfaceId) {
  ...
  try {
    ...
    if(colData.checkResponseSchema){
      ...
      console.log(interfaceData.res_body,interfaceData.res_body_is_json_schema )
      if(interfaceData.res_body_is_json_schema && interfaceData.res_body){
        let result = schemaValidator(interfaceData.res_body, context.body)
        if(!result.valid){
          throw (`返回Json 不符合 response 定义的数据结构:
          ${interfaceData.res_body}
          `)
        }
      }
    }
    console.log('checkResponseSchema:', colData.checkResponseSchema)
